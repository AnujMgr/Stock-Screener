...
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, Brush, Bar } from 'recharts';
import { useTheme } from 'next-themes';

const CustomAreaChart = ({ data, dataKeyForArea, showPeriodSelector, dataLength, showBush }) => {
  ...
  return (
    <Fragment>
      ...
      {showPeriodSelector ? (
        <PeriodSelector dataLength={dataLength} toggleTimeSpan={toggleTimeSpan} activeMenu={activeMenu} />
      ) : null}
      {/* </div> */}
        ...
        >
          <defs>
            <linearGradient id="color" x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="0%"
                stopColor={theme !== 'dark' ? '#b7e1c2' : '#2451B7'}
                stopOpacity={theme !== 'dark' ? 1 : 0.4}
              />
              <stop
                offset={theme !== 'dark' ? '85%' : '75%'}
                stopColor={theme !== 'dark' ? '#D1FAE5' : '#2451B7'}
                stopOpacity={theme !== 'dark' ? 0.1 : 0.05}
              />
            </linearGradient>
          </defs>
          ...
          <Area
            ...
            stroke={theme !== 'dark' ? '#03e08b' : '#2451B7'}
            fill="url(#color)"
            strokeWidth={1.6}
          />
          ...
          {showBush ? <Brush fill={theme !== 'dark' ? '#EFF6FF' : '#111827'} /> : null}
        </AreaChart>
      </ResponsiveContainer>
    </Fragment>
  );
        ...
        {payload.map(({ value, volume, payload }) => {
          return (
            <div key={value} className="bg-gray-800 p-2 text-white shadow-md">
              ...
              <p className="text-xs">
                Price: <span>{typeof value === 'number' ? value.toFixed(1) : (0).toFixed(1)}</span>
              </p>
              <p className="text-xs">
...
const PeriodSelector = ({ dataLength, toggleTimeSpan, activeMenu }) => {
  ...
  return (
    <div className="rounded-sm border border-gray-500 dark:border-gray-500 overflow-hidden">
      {dataLength >= 30 && (
        <button
          className={`px-3 py-1  ${activeMenu === '1m' && 'bg-blue-700 dark:bg-indigo-900 text-gray-100'}`}
      ...
      {dataLength >= 90 && (
        <button
          className={`px-3 py-1  ${activeMenu === '3m' && 'bg-blue-700 dark:bg-indigo-800 text-gray-100'}`}
      ...
      {dataLength >= 180 && (
        <button
          className={`px-3 py-1  ${activeMenu === '6m' && 'bg-blue-700 dark:bg-indigo-800 text-gray-100'}`}
      ...
      {dataLength > 360 && (
        <button
          className={`px-3 py-1  ${activeMenu === '1yrs' && 'bg-blue-700 dark:bg-indigo-800 text-gray-100'}`}
      ...
      {dataLength > 1800 && (
        <button
          className={`px-3 py-1  ${activeMenu === '5yrs' && 'bg-blue-700 dark:bg-indigo-800 text-gray-100'}`}
...
const CustomizedAxisTick = ({ x, y, payload, color }) => {
  const dateTip = moment(new Date(payload.value)).format('MMM YYYY').slice(0, 8);
  return (
